...
func (s *MysqlTagStore) Listar() ([]models.Tag, error) {
	...
	var rows, err = s.db.Query(`SELECT id, slug, nombre FROM tags`)
	if err != nil {
	...
	for rows.Next() {
		var nt models.Tag
		err = rows.Scan(&nt.Id, &nt.Slug, &nt.Nombre)
		if err != nil {
			return []models.Tag{}, err
		}
		tags[nt.Id] = nt
...
func (s *MysqlTagStore) Obtener(tag_id string) (models.Tag, error) {
	var tag models.Tag
	var row = s.db.QueryRow(`SELECT id, slug, nombre FROM tags WHERE id=?`, tag_id)
	var err = row.Scan(&tag.Id, &tag.Slug, &tag.Nombre)
	if err != nil {
		return models.Tag{}, err
	}
...
func (s *MysqlTagStore) ObtenerPorSlug(slug string) (models.Tag, error) {
	var tag models.Tag
	var row = s.db.QueryRow(`SELECT id, slug, nombre FROM tags WHERE slug=?`, slug)
	var err = row.Scan(&tag.Id, &tag.Slug, &tag.Nombre)
	if err != nil {
		return models.Tag{}, err
	}

	row = s.db.QueryRow(`SELECT COUNT(*) FROM publicaciones_tags WHERE tag_id=?`, tag.Id)
	err = row.Scan(&tag.Publicaciones)
	if err != nil {
		return models.Tag{}, err
	}
